...
class LogicLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.weights = torch.nn.parameter.Parameter(torch.randn(out_dim, 2, device=device))
        self.in_dim = in_dim
    ...
    def forward_cuda(self, x):
        ...
        if self.training:
            w = torch.nn.functional.sigmoid(self.weights).to(x.dtype)
            return LogicLayerCudaFunction.apply(
                x, a, b, w, self.given_x_indices_of_y_start, self.given_x_indices_of_y
            ).transpose(0, 1)
        else:
            w = (torch.nn.functional.sigmoid(self.weights) > 0.5).to(x.dtype)
            with torch.no_grad():
                return LogicLayerCudaFunction.apply(
                    x, a, b, w, self.given_x_indices_of_y_start, self.given_x_indices_of_y
                ).transpose(0, 1)
    ...
    def forward_cuda_eval(self, x: PackBitsTensor):
        ...
        a, b = self.indices
        w = (torch.nn.functional.sigmoid(self.weights) > 0.5).to(torch.uint8)
        x.t = diffAIG_cuda.eval(x.t, a, b, w)
...
class LogicLayerCudaFunction(torch.autograd.Function):
    ...
    def forward(ctx, x, a, b, w, given_x_indices_of_y_start, given_x_indices_of_y):
        ...
        return diffAIG_cuda.forward(x, a, b, w)
    ...
    def backward(ctx, grad_y):
        ...
        if ctx.needs_input_grad[0]:
            grad_x = diffAIG_cuda.backward_x(x, a, b, w, grad_y, given_x_indices_of_y_start, given_x_indices_of_y)
        if ctx.needs_input_grad[3]:
            grad_w = diffAIG_cuda.backward_w(x, a, b, w, grad_y)
